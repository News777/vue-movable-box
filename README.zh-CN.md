...
的一个子集：一到两个关键字（`left` / `center` / `right` / `top` / `bottom`）、百分比或 px 长度
（如 `'center'`、`'top left'`、`'50% 25%'`、`'10px 20px'`）；多余 token 会被忽略，`rem` /
`calc()` 不会被解析。旋转下的缩放会把指针与键盘位移映射到方框的本地坐标系（按逆角度旋转），
手柄沿旋转后的边缘放大或缩小。手柄本身采用"本地坐标系增量缩放"模型，而非精确的逆运动学锚定：
大角度下手柄的屏幕位移与指针路径不一致，但最小/最大尺寸、比例锁定、边界与碰撞约束仍按文档
语义执行。
