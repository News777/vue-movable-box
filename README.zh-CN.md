...
组合拖拽会话进行期间，选中成员上的缩放/旋转/键盘手势会被拒绝（而不是与 leader 的逐帧更新
互相覆盖；leader 自身的键盘同样被普通的"交互期间禁用键盘"规则拒绝）；正在执行自身手势的成员也不会被新开的会话编入编队。命令式调用（通过模板 ref 的
`setPosition`、`setSize`、`reset`）不参与该仲裁：它们是显式的程序化写入，会话期间可能被
leader 的下一帧覆盖。成员的 `memberId` 变化时会原地迁移注册：进行中的会话角色与选中状态都
跟随同一个组件实例到新 id。若新 id 已被另一个已挂载成员占用则拒绝改名（方框保留当前身份且
不会稍后自动重试；两个成员以相同 id 挂载时后者退回实例级身份），任何成员都不能悄悄顶替另一成员的注册。
...
- 强制中止（例如拖拽中切换 `disabled`）会直接结束会话、不还原，与单方框语义一致；引导方框
  在拖拽中被卸载同理（包括宿主整树卸载），其余成员停留在当前位置。这两条路径都会触发
  `source: null` 的 `move-cancel`，保证每个 `move-start` 都有终止事件。注意 `source` 只表示
  "没有 DOM 事件来源"：对 leader 命令式调用 `cancelInteraction()` 同样是 null 但会还原组合，
  因此不能仅凭载荷区分解散与还原——解散记录保留最后应用的矩形（尚未位移时才等于
  `startRect`），还原记录则始终等于 `startRect`。
- 会话开启时正在执行自身缩放/旋转的选中成员会保留在选中集合中，但不进入该会话的载荷
  （`rects` 中不含它）。
- 第二个并发指针无法抢占进行中的会话：未选成员仍可单独拖动；已在活动组合中的成员会拒绝第二次
  交互，确保进行中的组合不被改变。
- 组几何使用成员旋转后的视觉 AABB：共享边界按成员视觉轮廓的并集钳制，`sharedBounds: false`
  时各成员按自身视觉矩形钳制，旋转成员不会因外扩而超出区域边缘，编队形状保持不变。
