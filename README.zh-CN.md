...
transform-origin（`'center'`、`'top left'`、`'50% 50%'`）。旋转下的缩放会把指针与键盘位移映射
到方框的本地坐标系（按逆角度旋转），手柄沿旋转后的边缘放大或缩小。手柄本身采用"本地坐标系
增量缩放"模型，而非精确的逆运动学锚定：大角度下手柄相对光标可能有轻微漂移，但最小/最大尺寸、
比例锁定、边界与碰撞约束始终精确。

旋转几何语义（在 3.0.0 中定义）：

- **边界约束**以旋转后矩形的轴对齐包围盒（AABB）为准；`out-of-bounds` 同样按 AABB 判定。
  AABB 会考虑 `transformOrigin`，非 center 原点也按真实视觉位置钳制。
