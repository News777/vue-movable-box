...
function Ri(t, o) {
  return { handleKeyDown: (n) => {
    const s = t(), i = s.interacting;
    if (!i && (!s.enabled || s.disabled || !s.active)) return;
    if (n.key === "Escape") {
      n.preventDefault(), i ? o.cancel(n) : o.deactivate();
      return;
    }
    if (i || s.readOnly) return;
    const f = Mi[n.key];
    if (!f) return;
    const d = So(s.step);
...
const so = (t, o, a) => o.distance > a ? t : !t || o.distance < t.distance ? o : t, zi = ["alignment", "spacing"], lo = (t, o, a, n) => {
  ...
  const i = (h) => t === "horizontal" ? h.left : h.top, f = (h) => t === "horizontal" ? h.left + h.width : h.top + h.height, d = [], R = [];
  for (const h of s)
    f(h.rect) <= o && d.push(h), i(h.rect) >= o + a && R.push(h);
  let M = null;
  for (const h of d)
    for (const w of R) {
      const l = i(w.rect) - f(h.rect) - a;
      if (l < 0) continue;
...
function Ai(t, o, a = 10, n = { horizontal: !0, vertical: !0 }, s = {}) {
  const i = Math.max(0, Number.isFinite(a) ? a : 10), f = t.left + t.width, d = t.top + t.height, R = t.left + t.width / 2, M = t.top + t.height / 2, h = s.priority && s.priority.length > 0 ? s.priority : zi, w = h.includes("alignment"), l = h.includes("spacing"), O = (H, Q) => s.filter ? s.filter(H, Q) !== !1 : !0, N = /* @__PURE__ */ new Map(), z = (H) => {
    let Q = N.get(H);
    return Q || (Q = {
  ...
  const tt = () => {
    ...
    for (const et of o) {
      ...
      if (_.horizontal) {
        ...
        for (const zt of vt) H = so(H, zt, i);
      }
      if (_.vertical) {
        const vt = [
          {
        ...
        for (const zt of vt) Q = so(Q, zt, i);
      }
    }
    return { x: H, y: Q };
  ...
  const I = () => {
    ...
    i,
    p
  ) : co("vertical", t.top, t.height, i, v)) : null, W = n.horizontal ? lo("horizontal", i, h, {
    alignment: () => J("horizontal"),
    spacing: () => C("horizontal")
  }) : null, E = n.vertical ? lo("vertical", i, h, {
...
function Ti(t) {
  const o = (s) => {
    const i = t();
    return i.snapToGrid ? Si(s, i.gridSize) : s;
  }, a = (s, i) => ({
    left: o(s),
    top: o(i)
  }), n = bt(() => {
...
const Oe = (t) => {
  ...
  const i = (n - o) / s;
  return s > 0 ? { ...t, exit: Math.min(t.exit, i) } : { ...t, entry: Math.max(t.entry, i) };
}, fo = (t, o, a, n) => {
  const s = a - o;
  if (s === 0) return o > n ? t : null;
  const i = (n - o) / s;
  return s > 0 ? { ...t, entry: Math.max(t.entry, i) } : { ...t, exit: Math.min(t.exit, i) };
}, Oi = (t, o, a) => {
  ...
  const s = Math.max(0, n.entry), i = Math.min(1, n.exit);
  return s < i && i > 0 && s < 1 ? { entry: s, exit: i } : null;
};
function ve(t, o, a) {
  let n = null;
  for (const s of a) {
...
function Ei(t, o) {
  ...
  const s = t.left + t.width / 2, i = t.top + t.height / 2, f = o.left + o.width / 2, d = o.top + o.height / 2, R = s - f, M = i - d;
  return {
    colliding: !0,
...
const Ce = (t) => t.reduce((o, a) => o + (a.overlapArea ?? 0), 0), Z = (t) => {
  ...
  const n = Number.isInteger(a) && a >= 0 ? a : 0, s = 10 ** Math.min(n, 15), i = o * s, f = Math.max(Number.EPSILON, Math.abs(i) * Number.EPSILON * 8);
  return (o >= t ? Math.floor(i + f) : Math.ceil(i - f)) / s;
}, St = (t) => t * Math.PI / 180, Et = (t) => Math.round(t * 1e9) / 1e9, Ni = (t, o) => {
  const a = Z(o);
  if (a === 0) return { ...t };
  const n = St(a), s = Math.cos(n), i = Math.sin(n), f = Math.abs(t.width * s) + Math.abs(t.height * i), d = Math.abs(t.width * i) + Math.abs(t.height * s);
  return {
  ...
  const n = { x: o / 2, y: a / 2 }, i = To(t).split(" ");
  let f = null, d = null;
  const R = (M) => {
  ...
  if (s === 0) return n;
  const i = St(s), f = Math.cos(i), d = Math.sin(i), R = t.width / 2 - a.x, M = t.height / 2 - a.y, h = Et(R * f - M * d - R), w = Et(R * d + M * f - M);
  return {
    left: Et(n.left + h),
    top: Et(n.top + w),
    width: n.width,
  ...
  const s = St(n), i = Math.cos(s), f = Math.sin(s);
  return {
    x: Et(t * i + o * f),
    y: Et(-t * f + o * i)
  };
  ...
  for (const f of o) {
    ...
    d < i && (i = d, s = t + go(Z(f) - n));
  }
  return s === null || i > a ? t : Wi(s);
}, Wi = (t) => Math.abs(t) < 1e-9 ? 0 : t, Mt = 1e-7, Ne = (t) => Math.round(t * 1e9) / 1e9, ki = (t) => ({ x: Ne(t.x), y: Ne(t.y) }), Qt = (t) => {
  const o = Z(t.angle), a = St(o), n = Math.cos(a), s = Math.sin(a), i = t.origin.x, f = t.origin.y;
  return [
    { x: 0, y: 0 },
    { x: t.width, y: 0 },
  ...
  const o = Qt(t), a = o.map((f) => f.x), n = o.map((f) => f.y), s = Math.min(...a), i = Math.min(...n);
  return {
    left: s,
    top: i,
    width: Ne(Math.max(...a) - s),
    height: Ne(Math.max(...n) - i)
  };
  ...
  for (let a = 0; a < t.length; a += 1) {
    const n = t[(a + 1) % t.length], s = n.x - t[a].x, i = n.y - t[a].y, f = Math.hypot(s, i);
    f < Mt || o.push({ x: -i / f, y: s / f });
  }
  return o;
}, bo = (t) => ({
  ...
  for (let n = 0; n < o.length && a.length > 0; n += 1) {
    const s = o[n], i = o[(n + 1) % o.length], f = { x: i.x - s.x, y: i.y - s.y }, d = (w) => dn(f, { x: w.x - s.x, y: w.y - s.y }), R = a;
    a = [];
  ...
  for (const h of R) {
    ...
    const N = O < s - Mt, z = !N && O <= s + Mt && (i === null || Math.abs(h.x) > Math.abs(i.x));
    if (N || z) {
      s = Math.min(s, O);
  ...
  const o = Array.from(
    new Map(t.map((i) => [`${i.x},${i.y}`, i])).values()
  ).sort((i, f) => i.x === f.x ? i.y - f.y : i.x - f.x);
  if (o.length <= 2) return o;
  const a = (i, f, d) => (f.x - i.x) * (d.y - i.y) - (f.y - i.y) * (d.x - i.x), n = [];
  for (const i of o) {
    for (; n.length >= 2 && a(n[n.length - 2], n[n.length - 1], i) <= 0; )
      n.pop();
    n.push(i);
  }
  const s = [];
  ...
  for (const i of n)
    for (const f of a)
      s.push({ x: i.x - f.x, y: i.y - f.y });
  return Vi(s);
}, Yi = (t, o) => {
  if (o.length < 3) return null;
  let a = 0, n = 1;
  for (let s = 0; s < o.length; s += 1) {
    const i = o[s], f = o[(s + 1) % o.length], d = { x: f.x - i.x, y: f.y - i.y }, R = dn(d, t), M = dn(d, i);
    if (Math.abs(R) < Mt) {
      if (M > -Mt) return null;
      continue;
    }
  ...
  for (const i of a) {
    ...
    const M = Ki(t, i), h = Yi(o, M);
    h && (!n || h.entry < n.interval.entry) && (n = { interval: h, target: i, targetId: i.id });
  }
  return n;
}, Gt = (t, o) => ({
  ...t,
  ...
  for (const n of o) {
    ...
    const s = xn(n), i = Math.min(a.left + a.width, s.left + s.width) - Math.max(a.left, s.left), f = Math.min(a.top + a.height, s.top + s.height) - Math.max(a.top, s.top);
    if (!(i <= 0 || f <= 0) && He(t, n).overlapping)
      return !0;
  }
  return !1;
}, Oo = (t, o, a = 20) => {
  ...
  const n = (O, N) => Math.hypot(N.x - O.left - O.origin.x, N.y - O.top - O.origin.y), s = Math.max(
    ...Qt(t).map((O) => n(t, O)),
    ...Qt(o).map((O) => n(o, O))
  ), i = o.origin.x - t.origin.x, f = o.origin.y - t.origin.y, d = o.width - t.width, R = o.height - t.height, M = Math.max(
    Math.hypot(-i, -f),
    Math.hypot(d - i, -f),
    Math.hypot(-i, R - f),
    Math.hypot(d - i, R - f)
  ), h = Math.abs(o.left - t.left) + Math.abs(o.top - t.top) + Math.hypot(i, f) + M + s * St(Math.abs(o.angle - t.angle));
  ...
  for (let d = 0; d < 24; d += 1) {
    const R = (s + i) / 2, M = n(Co(t, o, R));
    ve(t, M, a) ? i = R : (f = M, s = R);
  }
  ...
  return o.forEach((n, s) => {
    ...
    const i = He(t, n);
    i.overlapping && a.set(s, i.overlapArea);
  }), a;
}, jt = (t, o) => {
  let a = 0;
  t.forEach((s) => {
...
function Qi(t) {
  const o = Ot([]), a = Ot(!1), n = (h) => (o.value = h, a.value = h.length > 0, ji(h)), s = (h, w) => {
    ...
    if (z.size > 0) {
      ...
      if (!jt(z, Rt(gt, l))) {
        const te = vt.results.length > 0 ? vt : i(h, l);
        return { accepted: !1, rect: h, progress: 0, ...te };
      }
      const zt = vt.results.length > 0 ? vt : i(gt, l);
      return {
        accepted: !Io(gt, h),
        rect: gt,
        progress: dt,
    ...
    const H = i(E, l), Q = H.results.length > 0 ? H : i(b, l), et = T.x * T.x + T.y * T.y > 0 ? ((E.left - h.left) * T.x + (E.top - h.top) * T.y) / (T.x * T.x + T.y * T.y) : 1;
    return {
      accepted: !Io(E, h),
      rect: E,
  ...
  return {
    ...
    resolveOrientedChange: (h, w, l) => {
      ...
      if (z.size > 0) {
        ...
        if (I >= 1)
          return { accepted: !0, rect: w, progress: 1, ...N };
        const C = ye(h, w, I), W = ye(h, w, I + (1 - I) * 1e-3), E = i(W, l);
        if (!jt(z, Rt(C, l))) {
          const rt = E.results.length > 0 ? E : i(h, l);
          return { accepted: !1, rect: h, progress: 0, ...rt };
        }
        const G = E.results.length > 0 ? E : i(C, l);
        return {
          accepted: I > 0,
          rect: C,
          progress: I,
      ...
      if (Po(h, w))
        return f(h, w, l);
      const T = Mo(h, w, l), tt = ye(h, w, T), J = i(tt, l);
      let p = J;
...
const x = (t, o = 0) => {
  ...
  const s = Jt(a, t.width, t.height), i = St(Z(o)), f = Math.cos(i), d = Math.sin(i), R = n.x - s.x, M = n.y - s.y;
  return {
    x: t.left + s.x + R * f - M * d,
    y: t.top + s.y + R * d + M * f
  };
}, Le = (t) => Math.abs(t) < 1e-9 ? 0 : t, fn = (t, o, a) => Math.min(Math.max(t, o), Math.max(o, a)), or = (t) => {
  const { start: o, angle: a, originSpec: n, handle: s, pointerDelta: i } = t, f = Z(a), d = er[s], R = gn(
    o,
    f,
    n,
  ...
  const i = Jt(a, t.width, t.height), f = bn(n, t.width, t.height), d = St(Z(o)), R = Math.cos(d), M = Math.sin(d), h = f.x - i.x, w = f.y - i.y;
  return {
    ...t,
    left: Le(s.x - i.x - (h * R - w * M)),
    top: Le(s.y - i.y - (h * M + w * R))
  };
...
const hn = (t, o = 1, a = ir) => {
  ...
  setup(t, { expose: o, emit: a }) {
    var Xn;
    const n = t, s = a, i = (e) => se(e), f = Ot(), d = Ot(i(n.modelValue)), R = Ot(Z(n.rotate)), M = i(n.modelValue), h = Ot(null), w = {
      ...
      initX: 0,
      initY: 0,
      beforeInteraction: i(n.modelValue),
    ...
    const O = bt(() => n.resizable ?? n.resizeable ?? !0), N = bt(() => n.unitType === "%"), z = bt(() => R.value), T = bt(() => To(n.transformOrigin)), tt = bt(() => ({
      ...
      const r = i(e);
      return d.value = r, s("update:modelValue", i(r)), r;
    }, b = (e) => {
      const r = L(Z(e)), u = r === -180 ? 180 : r;
      return R.value = u, s("update:rotate", u), s("rotate", u), u;
    };
    function I(e) {
      l.active !== e && (l.active = e, s(e ? "active" : "inactive", i(d.value)), e || _e());
    }
    const C = () => {
      var r, u, c;
      let e = null;
      ...
      l.parentElement = e ?? ((r = f.value) == null ? void 0 : r.parentElement) ?? null, l.parentWidth = ((u = l.parentElement) == null ? void 0 : u.clientWidth) ?? 0, l.parentHeight = ((c = l.parentElement) == null ? void 0 : c.clientHeight) ?? 0;
      ...
      const e = E(), r = N.value ? 100 : l.parentWidth, u = N.value ? 100 : l.parentHeight;
      return {
        minLeft: e.left,
        maxRight: Math.max(e.left, r - e.right),
        minTop: e.top,
      ...
      return {
        minLeft: r.minLeft,
        maxLeft: Math.max(r.minLeft, r.maxRight - e.width),
        minTop: r.minTop,
        maxTop: Math.max(r.minTop, r.maxBottom - e.height)
      };
    }, Y = (e) => ({
      left: x(e.left),
      top: x(e.top),
    ...
    }, et = (e, r = z.value) => Q(Y(e), r, n.transformOrigin);
    let F = null;
    const _ = () => {
      F = null;
    };
    ...
    const q = (e) => {
    ...
    }, ft = () => n.collisionEnabled && !n.allowOverlap && be.value, vt = (e, r, u) => {
      const c = Rt(et(r), u), m = (g) => jt(c, Rt(et(g), u));
      if (m(e)) return e;
      for (let g = 0.8; g > 0.01; g -= 0.2) {
        const y = {
          ...e,
          left: L(
            x(r.left) + (x(e.left) - x(r.left)) * g
          ),
          top: L(
            x(r.top) + (x(e.top) - x(r.top)) * g
          )
        };
        if (m(y)) return y;
      }
      ...
      const c = dt(), m = et(r), g = et(e), y = u === "slide" ? Me.resolveOrientedTranslation(m, g, c) : Me.resolveOrientedChange(m, g, c);
      ...
      return ft() ? vt(P, r, c) : P;
    }, te = () => n.collisionEnabled && !n.allowOverlap && be.value, Lo = (e) => {
      if (!n.limitAreaForParent || !l.parentElement) return !1;
      const r = G(), u = U(), c = e.left / u.x, m = e.top / u.y, g = e.width / u.x, y = e.height / u.y, A = 1e-7;
      return c < r.minLeft - A || c + g > r.maxRight + A || m < r.minTop - A || m + y > r.maxBottom + A;
    }, le = (e, r) => {
      const u = et(d.value, e), c = Fe(u);
      return Lo(c) ? !0 : !te() || r.length === 0 ? !1 : Do(u, r, c);
    }, Mn = (e, r, u) => Math.max(
      48,
      Math.ceil(Math.abs(r - e) / 2),
      Eo(
        et(d.value, e),
        et(d.value, r),
        u
      ...
      return c === 1 ? r : e + (r - e) * c;
    }, wn = (e, r) => {
      if (Math.abs(r - e) < 1e-9) return Z(r);
      const u = n.limitAreaForParent && !!l.parentElement, c = te() ? dt() : [];
      if (!u && c.length === 0) return Z(r);
      ...
      const g = Bo(e, r, c), y = Z(ho(e, g, m));
      return y !== g && le(y, c) ? Z(g) : y;
    }, In = (e, r) => {
      const u = z.value;
      if (!u && n.resizeMode !== "fixed-anchor" || !n.limitAreaForParent || !l.parentElement)
        return e;
      const c = G(), m = Math.max(0, c.maxRight - c.minLeft), g = Math.max(0, c.maxBottom - c.minTop), y = St(u), A = Math.abs(Math.cos(y)) < 1e-9 ? 0 : Math.abs(Math.cos(y)), P = Math.abs(Math.sin(y)) < 1e-9 ? 0 : Math.abs(Math.sin(y)), D = U(), k = A, B = P * (D.y / D.x), V = P * (D.x / D.y), j = A, X = x(e.width), K = x(e.height), S = r ? w[r] : null, at = x(e.left) + X, Nt = x(e.top) + K, At = (yt, It) => ({
        ...e,
        ...
        width: yt,
        height: It
      }), Wt = k * X + B * K, Yt = V * X + j * K, wt = Math.max(0, ut(n.minWidth, 0)), mt = Math.max(0, ut(n.minHeight, 0)), Xt = r === null || !!(S != null && S.left || S != null && S.right), Ht = r === null || !!(S != null && S.top || S != null && S.bottom), Tt = (S == null ? void 0 : S.left) ?? !1, ct = (S == null ? void 0 : S.top) ?? !1, nt = (yt, It) => {
        if (!Tt && !ct) return yt;
      ...
      const r = G(), u = H(e), c = u.left, m = u.top, g = c + u.width, y = m + u.height;
      c < r.minLeft && s("out-of-bounds", "left"), g > r.maxRight && s("out-of-bounds", "right"), m < r.minTop && s("out-of-bounds", "top"), y > r.maxBottom && s("out-of-bounds", "bottom");
    }, ke = (e) => {
      if (!n.limitAreaForParent || !l.parentElement) return e;
      const r = H(e), u = rt(r), c = $t(r.left, u.minLeft, u.maxLeft), m = $t(r.top, u.minTop, u.maxTop);
      return z.value ? {
        ...e,
        left: L(x(e.left) + (c - r.left)),
        top: L(x(e.top) + (m - r.top))
      } : {
        ...e,
        left: c,
        top: m
    ...
    const $e = (e) => Math.max(0, Math.floor(e * 1e6) / 1e6), Ve = {
      ...
      sharedDeltaProgress: (e, r) => {
        if (!n.collisionEnabled || n.allowOverlap) return 1;
        const u = U();
        if (be.value) {
          const P = dt(), D = et(e), k = { x: r.left * u.x, y: r.top * u.y }, B = Rt(D, P);
          if (B.size > 0) {
            if (!jt(B, Rt(Gt(D, k), P)))
              return 0;
            const j = pn(P, B);
    ...
    const Rn = () => $ ? n.snapTargets.filter((e) => !$.hasMember(e.id)) : n.snapTargets, Ke = () => {
      ...
      return $ ? e.filter((r) => !$.hasMember(r.id)) : e;
    }, L = (e) => n.isKeepDecimals ? hn(e, 0, n.decimalPlaces) : Math.round(e), Ye = (e, r) => {
      if (!N.value) return L(e);
      const u = r === "horizontal" ? l.parentWidth : l.parentHeight;
    ...
    const we = /* @__PURE__ */ new Set(["left", "right", "center-x"]), Ie = /* @__PURE__ */ new Set(["top", "bottom", "center-y"]), Xe = (e) => {
      ...
      const g = JSON.stringify({ guides: e.guides, targetIds: r });
      g !== ue && ((e.snapped || ue !== "clear") && s("guides", se(e.guides)), ue = e.snapped ? g : "clear");
    }, Tn = (e) => {
      const r = e.dominant, u = r ? {
        colliding: !0,
        direction: r.direction,
        targetId: r.targetId,
        normal: r.normal ? { ...r.normal } : void 0
      } : { colliding: !1 }, c = JSON.stringify(u);
      c !== fe && ((r || fe !== "clear") && s("collision", u), fe = r ? c : "clear");
    }, _e = () => {
      ...
      if (z.value) {
        if (u === "path" && m.progress !== void 0) {
          const g = $t(m.progress, 0, 1), y = Y(r), A = Y(e);
          return {
            ...e,
            left: L(
      ...
      return Xe(y), g;
    }, Ue = (e) => n.resizeDirections.includes(e), Dn = (e, r, u) => {
      if (n.resizeMode !== "fixed-anchor") {
        const B = Fi(u.x, u.y, z.value), V = {
          x: Ye(B.x, "horizontal"),
          y: Ye(B.y, "vertical")
        };
        return Fo(e, r, V.x, V.y);
      }
      ...
      if (k(P)) return P;
      const B = x(r.width), V = x(r.height), j = Math.max(0, ut(n.minWidth, 0)), X = Math.max(0, ut(n.minHeight, 0)), K = Math.min(B, j), S = Math.min(V, X), at = w[u], Nt = at.left || at.right, At = at.top || at.bottom, pt = n.ratioLock || Nt && At, Wt = pt ? Math.min(
        1,
        Math.max(
          B > 0 ? K / B : 0,
          V > 0 ? S / V : 0
      ...
      const m = w[r], g = x(e.left), y = x(e.top), A = x(e.width), P = x(e.height);
      ...
      if (n.ratioLock) {
        ...
        r === "tm" || r === "bm" || ee > ht ? Nt(S * at) : At(K / at);
      }
    ...
    let Ct = null, Vt = null;
    const En = (e, r, u) => Math.atan2(e.clientY - u, e.clientX - r) * 180 / Math.PI + 90, Cn = (e) => {
      ...
      const r = Sn(e.clientX - l.initX, "horizontal"), u = Sn(e.clientY - l.initY, "vertical"), c = i(d.value);
      if (l.isDragging) {
        const m = l.beforeInteraction;
        let g = x(m.left) + r, y = x(m.top) + u;
        const A = {
          horizontal: r < 0 && n.dragDirections.includes("left") || r > 0 && n.dragDirections.includes("right"),
        ...
        if (D && Bt && (D = ($ == null ? void 0 : $.constrainPosition(ot, D)) ?? null), D) {
          const k = v(D);
          s("move", i(k)), s("drag", i(k)), Bt && ($ == null || $.notifyMoved(ot, i(k)));
        }
      }
      ...
      if (!e) return;
      const r = { passive: !1 };
      ge(e, "pointermove", Nn, r), ge(e, "pointerup", Hn, r), ge(e, "pointercancel", Ln, r), ge(e, "keydown", Fn, !0);
      const u = f.value;
      u && ge(u, "lostpointercapture", Bn, r);
    }, ko = () => {
      const e = l.eventElement;
      if (!e) return;
      me(e, "pointermove", Nn, !1), me(e, "pointerup", Hn, !1), me(e, "pointercancel", Ln, !1), me(e, "keydown", Fn, !0);
      const r = f.value;
      r && me(r, "lostpointercapture", Bn, !1), l.eventElement = null;
    }, kn = () => {
      const e = f.value;
      ...
      const e = f.value, r = l.pointerId;
      if (l.pointerId = null, !(!e || r === null))
        try {
          e.hasPointerCapture(r) && e.releasePointerCapture(r);
        } catch {
        }
    };
    function Ze() {
    ...
    function he(e = null) {
      const r = l.isDragging, u = l.isResizing, c = l.isRotating, m = Bt;
      if (Ze(), Je(), r || u) {
        const g = i(l.beforeInteraction);
        v(g), s(r ? "drag-cancel" : "resize-cancel", e, g, i(g)), r && m && ($ == null || $.cancelDrag(ot, e));
      }
    ...
    const $o = (e, r) => {
      var c, m;
      if (n.disabled || n.initRect || l.isInteracting || r && (!O.value || !Ue(r)) || !r && !n.draggable) return;
      const u = i(d.value);
      if (r) {
        if (((c = n.canResize) == null ? void 0 : c.call(n, u, r)) === !1) return;
      ...
      C(), l.pointerId = typeof e.pointerId == "number" ? e.pointerId : null, l.initX = e.clientX, l.initY = e.clientY, l.beforeInteraction = i(d.value), l.handle = r, l.interactionMode = r ? "resize" : "drag", I(!0), l.isDragging && s("drag-start", e, i(l.beforeInteraction)), l.isResizing && s("resize-start", e, i(l.beforeInteraction)), l.eventElement = document.documentElement, Wn(), kn();
    }, Vo = () => {
      const e = f.value;
      if (!e) return null;
      const r = e.getBoundingClientRect(), u = e.offsetWidth || x(d.value.width), c = e.offsetHeight || x(d.value.height);
      if (!u || !c) return null;
      ...
      return {
        x: r.left - Math.min(...K.map((S) => S.x)),
        y: r.top - Math.min(...K.map((S) => S.y))
      };
    }, Ko = (e) => {
      var u;
      if (!e.isPrimary || e.button !== 0 || n.disabled || n.initRect || !n.rotatable || l.isInteracting || ((u = n.canRotate) == null ? void 0 : u.call(n, i(d.value))) === !1 || $ && !$.beginMemberInteraction(ot)) return;
      C();
      const r = Vo();
      r && (l.pointerId = typeof e.pointerId == "number" ? e.pointerId : null, l.beforeRotation = R.value, l.rotationOriginX = r.x, l.rotationOriginY = r.y, l.rotationStartPointerAngle = En(e, r.x, r.y), l.interactionMode = "rotate", I(!0), s("rotate-start", e, l.beforeRotation), l.eventElement = document.documentElement, Wn(), kn());
    }, Yo = (e) => {
      ...
      const u = (c) => {
        try {
          ...
          return {
            valid: !0,
            matched: m instanceof Element && r.contains(m)
          };
        } catch {
      ...
      !e.isPrimary || e.button !== 0 || !r && !Yo(e.target) || $o(e, r);
    };
    function Xo(e) {
      Ct !== null && (cancelAnimationFrame(Ct), Ct = null), Vt && (Cn(Vt), Vt = null), l.isDragging && (s("drag-stop", e, i(l.beforeInteraction), i(d.value)), Bt && ($ == null || $.endDrag(ot, e))), l.isResizing && s("resize-stop", e, i(l.beforeInteraction), i(d.value)), l.isRotating && s("rotate-stop", e, l.beforeRotation, R.value), $n();
    }
    const _o = (e, r) => {
      var g;
      if ($ && !$.beginMemberInteraction(ot)) return;
      C();
      const u = i(d.value);
      if (((g = n.canDrag) == null ? void 0 : g.call(n, i(u))) === !1) return;
      const c = i(u);
      e === "left" && (c.left = x(c.left) - r), e === "right" && (c.left = x(c.left) + r), e === "top" && (c.top = x(c.top) - r), e === "bottom" && (c.top = x(c.top) + r);
      const m = Pn(
        c,
        u,
        n.snapToElements,
      ...
      C();
      const c = i(d.value);
      if (((B = n.canResize) == null ? void 0 : B.call(n, i(c), e)) === !1) return;
      const m = r === "left" ? -u : r === "right" ? u : 0, g = r === "top" ? -u : r === "bottom" ? u : 0, y = { x: je(m), y: Qe(g) }, A = (() => {
      ...
      if (Kt(e)) return;
      const r = ze(e) ? n.maxWidth : n.maxHeight;
      if (r === void 0) return;
      const u = x(r);
      ...
      const r = Yn(e);
      if (r !== void 0)
        return n.unitType === "%" ? `${r} percent` : `${r} pixels`;
    }, ii = (e) => {
      ...
      const r = e.target, u = f.value;
      if (!(r instanceof Element) || !u || r === u || r.closest(".handle")) return !1;
      if (r.closest(".rotation-handle")) return e.key !== "Escape";
      const c = r.closest(ai);
      ...
      var y;
      if (!n.keyboardEnabled || n.disabled || n.initRect || !n.rotatable || !["ArrowLeft", "ArrowRight", "Home"].includes(e.key) || ((y = n.canRotate) == null ? void 0 : y.call(n, i(d.value))) === !1 || (e.preventDefault(), e.stopPropagation(), $ && !$.beginMemberInteraction(ot))) return;
      C();
      const r = R.value, u = So(n.keyboardStep) * (e.shiftKey ? 10 : 1), c = e.key === "Home" ? 0 : r + (e.key === "ArrowLeft" ? -u : u), m = e.key === "Home" ? c : mo(c, n.rotationSnapAngles ?? [], n.rotationSnapThreshold);
      s("rotate-start", e, r);
      const g = b(wn(r, m));
      s("rotate-stop", e, r, g);
    }, je = (e) => N.value ? e / 100 * l.parentWidth : e, Qe = (e) => N.value ? e / 100 * l.parentHeight : e, fi = bt(() => {
      const e = je(x(d.value.left)), r = Qe(x(d.value.top)), u = {
        left: `${-e}px`,
        top: `${-r}px`,
      ...
      if (c) {
        ...
        u.transform = `rotate(${-c}deg)`, u.transformOrigin = `${g.x + e}px ${g.y + r}px`;
      }
      return u;
    }), hi = (e) => ({
      left: `${je(e)}px`,
    ...
    return o({
      getConfig: () => i(d.value),
      setPosition: (e, r) => v({ ...d.value, left: e, top: r }),
      setSize: (e, r) => v({ ...d.value, width: e, height: r }),
      reset: () => v(i(M)),
      activate: () => I(!0),
      deactivate: Vn,
      ...
      style: Zt(tt.value),
      tabindex: "0",
      onPointerdown: r[1] || (r[1] = (u) => Kn(u, null)),
      onDblclick: r[2] || (r[2] = (u) => s("dblclick", u)),
      onFocus: ri,
      onKeydown: ci
    }, [
      Pe("div", {
  ...
  for (const [n, s] of o)
    a[n] = s;
  return a;
}, ur = /* @__PURE__ */ cr(lr, [["__scopeId", "data-v-af1563b6"]]), fr = Be({
  name: "MovableGroup"
}), hr = /* @__PURE__ */ Be({
  ...fr,
  props: {
  ...
  setup(t, { expose: o, emit: a }) {
    const n = t, s = a, i = /* @__PURE__ */ new Map(), f = Ot([]), d = Ot(null), R = bt(() => n.selected !== void 0), M = bt({
    ...
    const h = (p) => se(p), w = (p, v, b) => ({
      ...
      const v = p.filter((I) => i.has(I)), b = M.value;
      b.length === v.length && b.every((I, C) => I === v[C]) || (M.value = v);
    };
    return vi(No, {
      registerMember: (p, v) => i.has(p) && i.get(p) !== v ? !1 : (i.set(p, v), !0),
      renameMember: (p, v, b) => {
        if (i.get(p) !== b) return !1;
        if (p === v) return !0;
        if (i.has(v) && i.get(v) !== b) return !1;
        i.delete(p), i.set(v, b);
        const I = d.value;
      ...
      unregisterMember: (p) => {
        ...
        i.delete(p), b ? (d.value = null, s("move-cancel", { leaderId: p, source: null, rects: b })) : d.value && (d.value.startRects.delete(p), d.value.startVisuals.delete(p)), M.value.includes(p) && tt(M.value.filter((I) => I !== p));
      },
      hasMember: (p) => p !== void 0 && i.has(p),
      beginDrag: (p, v) => {
        if (d.value && d.value.leaderId !== p)
          return d.value.startRects.has(p) ? "blocked" : "solo";
        if (!i.has(p)) return "solo";
      ...
      constrainPosition: (p, v) => {
        ...
        if (n.sharedBounds) {
          ...
          for (const [U, H] of b.startRects)
            U !== p && ((rt = i.get(U)) == null || rt.translateTo(w(H, Y.left, Y.top)));
      ...
      cancelDrag: (p, v) => {
        ...
        for (const [W, E] of b.startRects)
          W !== p && ((C = i.get(W)) == null || C.translateTo(h(E)));
        const I = N(b.startRects);
      ...
      getSelected: () => [...M.value],
      select: (p) => tt(p ?? [...i.keys()]),
      getMemberRects: () => [...i.entries()].map(([p, v]) => ({ id: p, rect: h(v.getRect()) }))
