...
- 评审补充（2026-09-11）：第三方评审后补齐 FIX-08 文档项 —— 中英文 README 增加统一的 pnpm 9
  安装说明与 `pnpm exec playwright install chromium firefox webkit` 浏览器安装步骤；
  `pnpm test:package` 的 ESM/CJS 消费环境新增组件真实 SSR 挂载渲染断言。过程偏差记录：v3.2.0
  未按 2.5 节第 4 条先发布 `v3.2.0-beta` 预验证标签，直接进入正式版流程；截至记录时点
  v3.2.0–v3.5.0 的版本标签尚未创建。
...
- 评审记录：两轮独立代码评审（MiniMax-M3 与 DeepSeek），确认并修复 percent 模式 fixed-anchor
  ...
  边界优先）、组件层旋转 fixed-anchor 测试名实不符、Prettier 格式与文档小节位置等问题。
- 环境：Windows 10（win32 10.0.26200），Node 22.15，pnpm 9。
- 补充复审（2026-09-12）：对 FEAT-33-01 固定锚点缩放重新覆盖全部 8 个手柄、非中心原点、
  比例锁定与交叉 min/max、父级边界、精确碰撞、percent 单位、键盘和受控事件路径。修复比例
  冲突时突破最大尺寸、右/下侧靠边缩放导致锚点漂移、旋转后聚焦手柄无法沿本地轴键盘缩放三类
  问题；`pnpm test` 231 项、类型检查、生产构建、10 项 tarball 消费检查全部通过，三浏览器 e2e
  41 项通过、4 项按设计跳过。
- 补充复审（2026-09-13）：对 FEAT-33-03 角度吸附重新覆盖纯函数候选选择、非法候选与阈值、
  指针/键盘入口、`Home`、±180°、父级边界、AABB 兼容模式、精确碰撞、离散角度量化及越界恢复。
  修复 `Home` 被重新吸附、AABB 模式绕过旋转边界、安全小数角度四舍五入后重新违规、越界起点
  量化回违规侧四类问题；`pnpm test` 238 项、包消费检查 10 项全部通过，类型检查与生产构建通过，
  Playwright 三浏览器矩阵 41 项通过、4 项按平台能力设计跳过。
